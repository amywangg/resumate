...
applicant = Blueprint('applicant', __name__,
                      template_folder='templates/applicant')
...
def apply(job_id):
    ...
    cur.execute(
        "SELECT Skills FROM Skills s INNER JOIN Job j ON s.ID = j.Skills_ID WHERE j.ID =(%s)", [job_id])
    ...
    if request.method == 'POST':
        ...
        resumedata = ResumeParser(
            'resumetemp/' + resume.filename).get_extracted_data()
        ...
        file_id = drive.uploadFile(resume, folder_id, name + '-' + filename)

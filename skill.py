...
################################# JOB ROUTES #######################################
skill = Blueprint('skill', __name__, template_folder='templates')


# GET ALL JOBS
@skill.route('/skills', methods=['GET'])
def skills_page():
    ...
    data = cur.fetchall()
    cur.close()
    return render_template('skills.html', title="Skills", skills=data)


# CREATE JOB
# GET: get skills matrix if use existing, POST: post new job and skills
@skill.route('/skill/create', methods=['GET', 'POST'])
def create_job():
...
# DELETE JOB
@skill.route('/job/delete/<string:job_id>', methods=['GET'])
...
# UPDATE JOB
@skill.route('/job/update/<string:job_id>', methods=['POST'])
